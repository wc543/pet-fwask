...
beforeEach(async () => {
  await Promise.all(users.map(({ user_id, first_name, last_name, username, address, state, city, zip_code, phone_number, email, date_of_birth, hashed_password, role }) =>
    db.run(
  ...
  await Promise.all(pets.map(({ pet_id, name, type, breed, size, gender, age, color, created_by_id, fosterable, pet_image_url, shelter_time, current_foster, current_adopter, notes }) =>
    db.run(
